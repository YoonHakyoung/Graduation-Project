...
# read the CSV files
rodside_df = pd.read_csv('csv/Alcol(num)_seoul.csv')
adlist_df = pd.read_csv('csv/node(사당).csv')

# filter the node keys with Roadside value not equal to 0
roadside_node_keys = rodside_df.loc[rodside_df['Alcol'] != 0, 'node_key'].tolist()
