...
B = pd.read_csv('ALCOL.csv')

# 새로운 데이터프레임 생성을 위한 빈 리스트를 생성합니다.
result_list = []

# 데이터 프레임 A를 기준으로 주변 0.003 안에 B 값이 있는지 판별합니다.
...
# 결과 데이터프레임을 생성합니다.
result_df = pd.DataFrame(result_list, columns=['node_key', 'ALCOL'])
result_df['ALCOL'] = result_df['ALCOL'].apply(lambda x: 'True' if x else 'False')

# 결과 데이터프레임을 csv 파일로 저장합니다.
result_df.to_csv('ALCOL(TorF).csv', index=False)
